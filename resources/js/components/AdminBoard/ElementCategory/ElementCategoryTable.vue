...
<template>
  <v-card>
    ...
    <--=============================================================================================-->
    <table-toolbar :title="title" :total="noData" :add="setToggleDialog">
      <v-flex xs4>
        <v-text-field
          v-model="search"
          append-icon="search"
    ...
    <v-card-text>
      <v-data-table :headers="headers" :items="roleList.data" hide-default-footer>
        <!------------------------------------------------------------------------------------------- >
               		 	                  Actions
        <--------------------------------------------------------------------------------------------->
      ...
      <--=============================================================================================-->
      <table-pagination :meta="roleList.meta">
...
export default {
  ...
  computed: {
    ...mapGetters({
      roleList: "element_category/getItemList",
     ...
     */
    noData() {
      if (this.roleList.meta) {
        if (this.roleList.meta.total < 10)
          return "0" + this.roleList.meta.total;
        else return this.roleList.meta.total;
      } else {
        return "00";
      }
    },
    ...
    pages() {
      if (this.roleList.meta)
        return Math.ceil(
          this.roleList.meta.total / this.roleList.meta.per_page
        );
    }
  },
  methods: {
    ...mapActions({
      getroleList: "element_category/get_item_list",
      setToggleDialog: "element_category/set_toggle_form",
      setEditItem: "element_category/set_edit_item",
      deleteRole: "element_category/delete_given_item",
    }),
     ...
     */
    searchItem() {
      this.getroleList({ page: this.page, query: this.search });
    }
  }
};
</script>